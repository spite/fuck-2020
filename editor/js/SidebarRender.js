...
function SidebarRender(editor) {
  ...
  render.onClick(async () => {
    ...
    if (audio !== null) {
      await ffmpeg.run(
        `-framerate ${fps} -pattern_type glob -i *.png -i audio.mp3 -c:a aac -shortest -c:v libx264 -pix_fmt yuv420p -preset slow -crf 0 out.mp4`,
        { output: "out.mp4" }
      );
    } else {
      await ffmpeg.run(
        `-framerate ${fps} -pattern_type glob -i *.png -c:v libx264 -pix_fmt yuv420p -preset slow -crf 0 out.mp4`,
        { output: "out.mp4" }
      );
    }
